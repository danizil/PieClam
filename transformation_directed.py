...
def relu_lightcone(xt, grad, lr, node_mask, cutoff=0.0):
    '''does relu to the point update on the lightcone. coordinates are rotated->relu->rotated back. we juxtabpose the time and space features. then rotate them with the rotation matrix.
    For directed graphs the same is done only for the sender and receiver separately'''
    #todo: replace the first lines of code with the function uv_from_xt
    # i put the graph through the clamiter which iterates both sender and reveiber features
