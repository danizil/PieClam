...
\begin{align}
    ...
    \prod_{m=1}^{N}e^{-\mathbf{f}_n^T \mathbf{L} \mathbf{f}_m} 
    ...
    &\quad \times \prod_{n=N+1}^{2N}\Bigg(
        \prod_{m=N+1}^{2N}e^{-\mathbf{f}_n^T \mathbf{L} \mathbf{f}_m} 
...
By creating a second set of nodes, we in fact create a second set of features for each of the nodes. We may use this intuition and add this second set directly. Each node will now have two feature sets $\mathbf{s} \in \mathbb{R}^{N \times d}$ (sender) and $\mathbf{r} \in \mathbb{R}^{N \times d}$ (receiver).

Rewrite \ref{eq:big_bipartite} as:
\begin{align}
    p(E|F) &= \prod_{n=1}^{N} \Bigg[\sqrt{
    \bigg(
    \prod_{m=1}^{N}e^{-\mathbf{s}_n^T \mathbf{L} \mathbf{s}_m} 
    \prod_{m \leftarrow n}\Big(1-e^{-\mathbf{s}_n^T \mathbf{L} \mathbf{r}_m}\Big)
    \prod_{m \not\leftarrow n}e^{-\mathbf{s}_n^T \mathbf{L} \mathbf{r}_m}
    \bigg)} \notag \\
    &\quad \times \sqrt{\bigg(
        \prod_{m=1}^{N}e^{-\mathbf{r}_n^T \mathbf{L} \mathbf{r}_m} 
        \prod_{m \rightarrow n}\Big(1-e^{-\mathbf{r}_n^T \mathbf{L} \mathbf{s}_m}\Big)
        \prod_{m \not\rightarrow n}e^{-\mathbf{r}_n^T \mathbf{L} \mathbf{s}_m}
        \bigg)
    }\Bigg]
\end{align}
...
\begin{align}
    p(E|F) &= \prod_{n=1}^{N} \Bigg(
        \prod_{m=1}^{N}e^{-\frac{1}{2}(\mathbf{s}_n^T \mathbf{L} \mathbf{s}_m + \mathbf{r}_n^T \mathbf{L} \mathbf{r}_m)}
        \prod_{m \rightarrow n}\Big(1-e^{-\mathbf{r}_n^T \mathbf{L} \mathbf{s}_m}\Big)
        \prod_{m \not\rightarrow n}e^{-\mathbf{r}_n^T \mathbf{L} \mathbf{s}_m}
    \Bigg)
\end{align}
...
\begin{align}
    \log p(E|F) &= \sum_{n=1}^{N} \Bigg(
        \sum_{m=1}^{N} -\frac{1}{2}(\mathbf{s}_n^T \mathbf{L} \mathbf{s}_m + \mathbf{r}_n^T \mathbf{L} \mathbf{r}_m) +
        \sum_{m \rightarrow n}^{N} \log(1-e^{-\mathbf{s}_n^T \mathbf{L} \mathbf{r}_m}) -
        \sum_{m \not\rightarrow n}^{N} \mathbf{s}_n^T \mathbf{L} \mathbf{r}_m
    \Bigg)
\end{align}
...
\begin{align}
    \log p(E|F) &= \sum_{n=1}^{N} \Bigg(
        \sum_{m=1}^{N} -\frac{1}{2}(\mathbf{s}_n^T \mathbf{L} \mathbf{s}_m + \mathbf{r}_n^T \mathbf{L} \mathbf{r}_m + 2 \mathbf{s}_n^T \mathbf{L} \mathbf{r}_m) +
        \sum_{m \rightarrow n}^{N} \log(e^{\mathbf{r}_n^T \mathbf{L} \mathbf{s}_m} - 1) 
    \Bigg) \notag\\
    &= \sum_{n=1}^{N} \Bigg(
        \sum_{m=1}^{N} -\frac{1}{2}(\mathbf{s}_n + \mathbf{r}_n)^T \mathbf{L} (\mathbf{s}_m + \mathbf{r}_m) +
        \sum_{m \rightarrow n}^{N} \log(e^{\mathbf{r}_n^T \mathbf{L} \mathbf{s}_m} - 1) 
    \Bigg)
\end{align}

The gradient updates are then:
\begin{align}
    \nabla_{\mathbf{s_n}}l &= -\frac{1}{2} \sum_{m=1}^{N} \mathbf{L} (\mathbf{s}_m + \mathbf{r}_m) +
    \sum_{m \leftarrow n}^{N} \frac{\mathbf{L} \mathbf{r}_m}{e^{\mathbf{r}_n^T \mathbf{L} \mathbf{s}_m} - 1} \\
    \nabla_{\mathbf{r_n}}l &= -\frac{1}{2} \sum_{m=1}^{N} \mathbf{L} (\mathbf{s}_m + \mathbf{r}_m) +
    \sum_{m \rightarrow n}^{N} \frac{\mathbf{L} \mathbf{s}_m}{e^{\mathbf{r}_n^T \mathbf{L} \mathbf{s}_m} - 1}
\end{align}
